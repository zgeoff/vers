...
`baseEnvSchema` and validated against `process.env` at boot. A project that also needs a local `.env`
ships a committed `.env.example` documenting its keys; real values come from `bun run env:pull`,
which pulls each managed file from the `vers` 1Password vault via the `op` CLI. `.env` files
