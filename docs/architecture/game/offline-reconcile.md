...
A start waits on the worker's first reconstruction of its avatar since the worker booted. The worker
refuses the start as `unreconstructed` when that resync fails or its rebuilt checkpoint stream
diverges from the server's, and the next resync that completes lifts the refusal.
