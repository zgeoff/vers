...
Each cross-cutting concern in vers runs on one dedicated external service, fronted by a
vendor-neutral interface: OpenTelemetry for telemetry, OpenFeature for flags, the Sentry ingest
protocol for errors. Swapping the backend behind an interface changes configuration, never call
sites. Single-purpose providers beat a bundled platform: the app owns the pipe, and the provider
owns one job. The catalog below names the provider behind each concern and the doc that owns its
wiring.
