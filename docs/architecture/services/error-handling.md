...
| activity     | `AVATAR_NOT_ACTIVE`    | 409    | Start or reveal refused because the acting avatar is not the account's active one                                                                                                                                                                                                                                       | `{ activeAvatarID, activeAvatarName }` |
...
| activity     | `CHECKPOINT_INVALID`   | 422    | Checkpoint batch fails structural or cross-check validation (contiguity, chainIndex, chain link, time monotonicity, hash, or — `advanceActivity` only — a mismatch between the client's predicted build snapshot and the server-authored one). `advanceActivity` carries `activityID`/`appendedHead` alongside `reason` | `{ reason }`                           |
| activity     | `NODE_NOT_REVEALED`    | 409    | Start's scope has no chain row — `revealNodes` was never called for it, so there is nothing to root the new activity against                                                                                                                                                                                            | —                                      |
| activity     | `NODE_UNKNOWN`         | 404    | Start's or `revealNodes`'s scope id doesn't resolve to a node on the current world map                                                                                                                                                                                                                                  | —                                      |
| activity     | `NODE_UNREACHABLE`     | 409    | Start's scope node resolves but sits outside the avatar's selectable set — neither the origin, a completed node, nor a neighbour of one                                                                                                                                                                                 | —                                      |
...
| activity     | `SIM_VERSION_EXPIRED`  | 410    | Stamped or current sim version is past retention, or its engine's max content version falls behind the content the activity would stamp — the client must resync onto the current version, or reload for the latter                                                                                                     | `{ currentSimVersion }`                |
| activity     | `SIM_VERSION_UNKNOWN`  | 409    | Stamped or current sim version isn't registered — on a pinned hash the client retries once without it, landing on the registry-current stamp (a deploy can bake a hash whose registry row hasn't landed yet)                                                                                                            | `{ currentSimVersion }`                |
