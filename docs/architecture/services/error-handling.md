...
| activity     | `CHAIN_QUARANTINED`    | 409    | New start, or `advanceActivity` mint, refused while the chain's replay frontier is quarantined; the bulk endpoint carries `{ activityID, appendedHead }` naming its last committed row                                                                                                                                  | —                                      |
| activity     | `CHECKPOINT_INVALID`   | 422    | Checkpoint batch fails structural or cross-check validation (contiguity, chainIndex, chain link, time monotonicity, hash, or — `advanceActivity` only — a mismatch between the client's predicted build snapshot and the server-authored one). The bulk endpoint carries `activityID`/`appendedHead` alongside `reason` | `{ reason }`                           |
