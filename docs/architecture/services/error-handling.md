...
| ------------ | ---------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| activity     | `ACTIVITY_CAPPED`      | 409    | Append exceeds the avatar's accrued offline-progress budget — the activity lands terminal `capped`; the client resyncs and rebases at the returned stop index. `advanceActivity` carries the same data plus `activityID`, naming the bulk request's last fully committed row                                                                                                      | `{ appendedHead }`                     |
| activity     | `ACTIVITY_TERMINAL`    | 409    | Append against a terminal activity status — fatal for the stream, the client discards. `advanceActivity` carries the same data plus `activityID`                                                                                                                                                                                                                                  | `{ status, appendedHead }`             |
| activity     | `AVATAR_NOT_ACTIVE`    | 409    | Start, reveal, or an `advanceActivity` root mint refused because the acting avatar is not the account's active one                                                                                                                                                                                                                                                                | `{ activeAvatarID, activeAvatarName }` |
| activity     | `CHAIN_QUARANTINED`    | 409    | New start, or `advanceActivity` mint, refused while the chain's replay frontier is quarantined; `advanceActivity` carries `{ activityID, appendedHead }` naming its last committed row                                                                                                                                                                                            | —                                      |
| activity     | `CHECKPOINT_INVALID`   | 422    | Checkpoint batch fails structural or cross-check validation (contiguity, chainIndex, chain link, time monotonicity, hash, or — `advanceActivity` only — a mismatch between the client's predicted build snapshot and the server-authored one, or, for a root mint, a start-hash recomputation mismatch). `advanceActivity` carries `activityID`/`appendedHead` alongside `reason` | `{ reason }`                           |
| activity     | `NODE_NOT_REVEALED`    | 409    | Start's, or an `advanceActivity` root mint's, scope has no chain row — `revealNodes` was never called for it, so there is nothing to root the new activity against                                                                                                                                                                                                                | —                                      |
| activity     | `NODE_UNKNOWN`         | 404    | Start's, `revealNodes`'s, or an `advanceActivity` root mint's scope id doesn't resolve to a node on the current world map                                                                                                                                                                                                                                                         | —                                      |
| activity     | `NODE_UNREACHABLE`     | 409    | Start's scope node resolves but sits outside the avatar's selectable set — neither the origin, a completed node, nor a neighbour of one. An offline-reconciled root mint is admitted regardless; reachability is adjudicated at replay instead                                                                                                                                    | —                                      |
| activity     | `SESSION_EVICTED`      | 403    | Append or stop from a session that is no longer the activity's writer — fatal for that session's stream, the client discards. `advanceActivity` carries `{ activityID, appendedHead }` instead of the empty payload                                                                                                                                                               | —                                      |
| activity     | `SIM_VERSION_EXPIRED`  | 410    | Stamped or current sim version is past retention, or its engine's max content version falls behind the content the activity would stamp — the client must resync onto the current version, or reload for the latter. An `advanceActivity` root mint validates its own stamped version the same way                                                                                | `{ currentSimVersion }`                |
| activity     | `SIM_VERSION_UNKNOWN`  | 409    | Stamped or current sim version isn't registered — on a pinned hash the client retries once without it, landing on the registry-current stamp (a deploy can bake a hash whose registry row hasn't landed yet). An `advanceActivity` root mint validates its own stamped version the same way                                                                                       | `{ currentSimVersion }`                |
