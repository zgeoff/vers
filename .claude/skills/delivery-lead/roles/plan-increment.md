...
2. Membership. For each open issue in each open increment, ask whether the done test fails without
...
4. Close. When an increment's open count reaches 0, or every remaining issue is proposed out,
   propose closing the milestone. When the lead increment closes, the next `P` number leads.
5. Shape. Run the membership pass on every open increment, lead first, so the owner sees each one's
   shape while agents pick from it.
