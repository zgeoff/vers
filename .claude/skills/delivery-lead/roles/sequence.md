...
1. Edges. For each open issue in an open increment, check that every dependency its body names has
   an edge, and add the missing ones. Propose removing an edge the body contradicts.
2. Critical path. For each open issue in an open increment, count the open issues it transitively
