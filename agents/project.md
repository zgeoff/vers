...
- `bun run deadcode` — knip over the whole workspace (`knip.json` at the root; `scripts/run-knip.ts`
  writes a throwaway root `vite.config.ts` stub that knip's Vite-family plugins insist on loading).
  Blocking in CI and pre-push. It needs codegen output present — a missing router typegen reports as
  an unresolved import. A dependency knip can't see (runtime-string imports, type-only peers, test
  preloads, css-imported fonts) gets a `knip.json` ignore in the PR that introduces it.
