...
- Test data states exactly the fields the behaviour and assertions depend on. Scalar and small
  ...
  object, each factory with its own test), called inline with overrides for the asserted fields — an
  explicit override marks the field as relevant to the unit under test, and factory defaults keep
  the rest out of the test body. Module-level fixtures shared between tests are a defect.
