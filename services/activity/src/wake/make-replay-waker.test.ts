...
test('it never throws into the caller', () => {
  const waker = makeReplayWaker({ ...IMMEDIATE, sendWake: mock(() => Promise.resolve()) });
...
test('it coalesces a synchronous burst into one delivery', async () => {
  ...
  const waker = makeReplayWaker({ ...IMMEDIATE, coalesceWindowMs: 1000, sendWake });
...
test('it keeps a single delivery in flight while one is still pending', async () => {
  ...
  const waker = makeReplayWaker({ ...IMMEDIATE, sendWake });
