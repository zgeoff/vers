...
/**
 * Resolves the test container's base URI and migrated template DB name, published to the
 * `TEST_DB_URI`/`TEST_TEMPLATE_DB` env vars once per test process. Falls back to the test
 * container's fixed defaults so a bare `bun test` against an already-running container still
 * resolves.
 */
