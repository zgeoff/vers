...
import type { ActivityAppState, ActivityData, AvatarData } from '@vers/idle-core';
...
/**
 * The app's one seam onto `lib-idle-client`'s SharedWorker mount: every consumer reads simulation
 * state through this hook instead of the library directly, so tests can stub the worker handle at
 * this boundary. `happy-dom` has neither `SharedWorker` nor the Vite worker-import transform the
 * library's own (still-vitest) tests depend on, so this module is replaced wholesale under
 * `bun test` rather than driven for real — see `test-utils/register-idle-worker-handle-mock.ts`.
 */
