...
test('it shows validation errors for missing required fields', async () => {
  const ctx = setupTest({ isAuthed: false, isOnboarding: true });

  const createAccountButton = await screen.findByRole('button', {
    name: 'Create an Account',
  });

  await ctx.user.click(createAccountButton);
...
test('it redirects to the nexus on successful account creation', async () => {
  ...
  await ctx.user.type(usernameInput, 'testuser');
  await ctx.user.type(nameInput, 'Test User');
  await ctx.user.type(passwordInput, 'password123');
  await ctx.user.type(confirmPasswordInput, 'password123');
  await ctx.user.click(agreeToTermsCheckbox);
  await ctx.user.click(createAccountButton);
