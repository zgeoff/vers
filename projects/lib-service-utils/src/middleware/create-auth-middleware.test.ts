...
test('it authorizes a valid token and extracts the payload and user ID', async () => {
  const setup = await setupTest({ isAuthRequired: true });

  const req = new Request('http://localhost/test');

  req.headers.set('Authorization', `Bearer ${setup.token}`);

  const res = await setup.app.request(req);
...
test('it allows requests without auth header when auth is optional', async () => {
  const setup = await setupTest({ isAuthRequired: false });
