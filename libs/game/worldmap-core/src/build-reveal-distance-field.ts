 ...
 * inside its hex disc — so every genuinely revealed cell sits fully inside the clear region and
 * fog never hides opened ground.
 *
...
function collectReachingDiscs(
  ...
  for (const source of sources) {
    const clearRadius = source.radius * SQRT_3;
