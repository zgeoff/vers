...
function registerSharedWorkerStub() {
  ...
  Reflect.set(globalThis, 'SharedWorker', MockSharedWorker);
...
test('it initializes the worker connection', () => {
  ...
  expect(hook.result.current).toBeInstanceOf(MockSharedWorker);

  hook.unmount();
});

test('it returns an existing worker instead of creating a new one', () => {
  // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- MockSharedWorker stands in for a real SharedWorker; the hook only ever touches its `.port`
...
test('it updates simulation state from worker messages', async () => {
  ...
  // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- the hook was stubbed to construct a MockSharedWorker, so its return value has that shape at runtime
  const worker = hook.result.current as unknown as MockSharedWorker;
