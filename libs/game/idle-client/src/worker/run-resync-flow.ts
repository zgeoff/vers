...
async function applyFastForward(
  ...
  if (report.finalRowTerminal) {
    ...
    // policy's online counterpart never starts a successor at all — stop this one back durably so
    // the gap reads idle server-side too, instead of sitting active for the next resync to revive.
