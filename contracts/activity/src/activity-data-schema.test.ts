...
test('it rejects an activity with a null secretVersion', () => {
  const result = ActivityDataSchema.safeParse({
    appendedAt: null,
    appendedHead: 0,
    avatarID: 'avatar_1',
...
test('it rejects an activity with a null secretRef', () => {
  const result = ActivityDataSchema.safeParse({
    appendedAt: null,
    appendedHead: 0,
    avatarID: 'avatar_1',
  ...
  expect(result.success).toBeFalse();
  expect(result.error?.issues).toPartiallyContain(expect.objectContaining({ path: ['secretRef'] }));
});
